...
function gradePart1() {
  ...
  for (let i = 0; i < PART1_QUESTIONS.length; i++) {
    ...
    const correct = selected && parseInt(selected.value, 10) === ANSWERS.p1[i];
    if (correct) score += 0.25;
    detail.push({ index: i, answered: !!selected, correct: !!correct });
  }
  return { score, detail };
}
...
function gradePart2() {
  ...
  for (let q = 1; q <= PART2_QUESTIONS.length; q++) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (selected && selected.value === ANSWERS.p2[q - 1][i]) {
        correctCount++;
      }
    }
    let qScore = 0;
    ...
    score += qScore;
    detail.push({ index: q - 1, correctCount, score: qScore });
  }
  return { score, detail };
}
...
function gradePart3() {
  ...
  for (let i = 1; i <= PART3_QUESTIONS.length; i++) {
    ...
    const correct = value !== "" && value === ANSWERS.p3[i - 1];
    if (correct) score += 0.25;
    detail.push({ index: i - 1, value, correct });
  }
  return { score, detail };
}
